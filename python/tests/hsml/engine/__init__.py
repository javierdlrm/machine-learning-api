...
def mock_find_spec(name, package=None):
    return pydoop_available if name == "pydoop" else find_spec(name, package)


class TestModelEngine:
    @patch("importlib.util.find_spec", new=mock_find_spec)
    def _copy_or_move_hopsfs_model():
        pass

    @patch("importlib.util.find_spec", new=mock_find_spec)
    def save_relative_path_with_pydoop(self, mocker: MockerFixture):
        global pydoop_available
        pydoop_available = True

        mock_local_engine_upload = mocker.patch(
            "hsml.engine.local_engine.LocalEngine.upload"
        )
        mock_local_engine_mkdir = mocker.patch(
            "hsml.engine.local_engine.LocalEngine.mkdir"
        )
        mock_hw_engine_upload = mocker.patch(
            "hsml.engine.hopsworks_engine.LocalEngine.upload"
        )
        mock_hw_engine_mkdir = mocker.patch(
            "hsml.engine.hopsworks_engine.LocalEngine.mkdir"
        )
        model = Model(id=None, name="testmodel", version=1, project_name="testproject")
        me = model_engine.ModelEngine()

        me.save(model, "testdir")

        assert mock_local_engine_upload.call_count == 0
        assert mock_local_engine_mkdir.call_count == 0
        assert mock_hw_engine_upload.call_count == 0
        assert mock_hw_engine_mkdir.call_count == 0

    @patch("importlib.util.find_spec", new=mock_find_spec)
    def save_abs_path_with_pydoop(self, mocker: MockerFixture):
        global pydoop_available
        pydoop_available = True
